...
export class BottomSheetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data.objectLists) {
      // Alle initial auf den ersten Eintrag setzen
      this.objectLists = this.data.objectLists.map((item) => item.data[0]);

      // Initial nichts setzen
      // this.objectLists = new Array(this.data.objectLists.length).fill("");
    }
    ...
    if (this.data.timeInputs) {
      this.data.timeInputs.forEach((timeInput) => {
        ...
        this.formGroup.controls["timeInput" + timeInput.placeholder].valueChanges.subscribe(
          (value) => {
            this.disableAllButtonInputsIfNullValue(+value);
          }
        );
  ...
  getAllData() {
    const data: DialogReturnData = {
      ...
      objectListInputs: this.objectLists,
    };
  ...
  onListActionDeleteAllExceptFirstClick(index: number) {
    if (this.data.itemsInputs) {
      this.data.itemsInputs[index].items.splice(1);
    }
  }

  onListActionDeleteAllExceptFirstTwoClick(index: number) {
    if (this.data.itemsInputs) {
      this.data.itemsInputs[index].items.splice(2);
    }
  }

  onListActionClick(buttonIndex: number, index: number) {
    if (
      this.data.itemsInputs &&
      this.data.itemsInputs[index]?.buttons![buttonIndex]?.closeAfterClick
    ) {
