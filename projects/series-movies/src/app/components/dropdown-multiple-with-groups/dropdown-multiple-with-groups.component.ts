...
export class DropdownMultipleWithGroupsComponent<EntryType>
  implements OnInit, AfterViewInit, OnDestroy, OnChanges
{
  @Input() value: EntryType[] = [];
  @Input() formfieldKey: string;
  @Input() formfieldName: string;
  @Input() extraIcons?: string[];
  @Input() noneText: string;
  @Input() noneValue: EntryType; // 0
  @Input() noneIcon: string;
  @Input() noText?: string;
  @Input() noValue?: EntryType; // -1
  @Input() noIcon?: string;
   ...
   */
  @Input() specialOppositeValues: [EntryType, EntryType][] = [];
  ...
  selectedEntries: EntryVM<EntryType>[];

  lastSelected: EntryType;
  ...
  get selectedValue() {
    const noneValue = this.noneValue ?? (0 as EntryType);
    return this.selected.length > 0 && this.selected[0].type !== noneValue;
  }

  get noOrNoneValueSet() {
    const noneValue = this.noneValue ?? (0 as EntryType);
    const noValue = this.noValue ?? (-1 as EntryType);
  ...
  onSelectChanged(event: EntryType[]) {
    this.value = event;
  ...
  fillSelectedTextsAndIcons() {
    ...
    types.forEach((selectedEntry) => {
      selectedEntries.push({
        ...
        type: selectedEntry.type ?? this.noneValue ?? (0 as EntryType),
        ...
        : [
            {
              name: this.noneText ?? "",
              icon: this.noneIcon,
              type: this.noneValue ?? (0 as EntryType),
            },
          ];
  }

  onRemoveFromSelection(event: any, entry: EntryVM<EntryType>): void {
    ...
    if (entry.isGroup) {
      ...
      if (newSelected.length === 0) {
        newSelected = [this.noneValue ?? (0 as EntryType)];
      }
  ...
  onGroupClick(groupElement: HTMLElement, group: Group<EntryType>) {
    // Überprüfen ob "selected" bereits alle Einträge der Gruppe enthält
    const allEntriesSelected = group.entries.every((e) =>
      this.value.includes(e.type ?? this.noneValue ?? (0 as EntryType))
    );
  ...
  private onOptionClickProofForHeaderHighlightChange(
    optionClickedType?: EntryType,
    allNewSelected?: EntryType[]
  ) {
    ...
    if (index !== -1) {
      ...
      const allEntriesSelected = this.groups[index].entries.every((e) => {
        return allNewSelected.includes(e.type ?? this.noneValue ?? (0 as EntryType));
      });
   ...
   */
  private setSelectedAndTriggerOnChange(newSelected: EntryType[]) {
  ...
  private setSelectedNoneAndTriggerOnChange() {
    this.value = [this.noneValue ?? (0 as EntryType)];
    this.onChange.emit([this.noneValue ?? (0 as EntryType)]);
  }
   ...
   * Alle Einträge der Gruppe aus der Auswahl entfernen
   */
  private removeAllEntriesFromSelected(selected: EntryType[], group: Group<EntryType>) {
